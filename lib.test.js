...
test('entries of fixed and computed values', async () => {
    const res = await compute({'prefix_key1': 'value', 'prefix_key2': 'key1=${key1}'}, 'prefix_', BASH);
...
test('entries of fixed and computed values in reverse lexicographic order', async () => {
    const res = await compute({'prefix_key2': 'value', 'prefix_key1': 'key2=${key2}'}, 'prefix_', BASH);
    expect(res).toEqual({'key2': 'value', 'key1': 'key2=value'});
});

test('entries of fixed and computed values, referencing before defining', async () => {
    const res = await compute({'prefix_key2': 'key1=${key1}', 'prefix_key1': 'value'}, 'prefix_', BASH);
...
test('substitutions are not HTML escaped', async () => {
    const res = await compute({'prefix_key1': 'val=ue', 'prefix_key2': 'key1=${key1}'}, 'prefix_', BASH);
